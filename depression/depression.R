...
fit_our_exp <- fsnet::fsnet(Y = log(Y),
                            X = model.matrix(score ~ ., data = use_dat),
                            lam = 0,
                            fix_var = T,
                            maxit = rep(1e3, 3),
...
for(ii in 0:27){
  ...
  pdf[ii + 1, ] <- exp(c(fsnet:::loglik_ab(a = a_male, b = b_male, order = 0, dist = 1),
                     fsnet:::loglik_ab(a = a_fem, b = b_fem, order = 0, dist = 1)))
}
